...
class FunctionCallDataSender(object):
    ...
    def on_function_call(self, frame, event, arg):
        if event == 'return':
            try:
                data = (self._get_persisted_object(frame.f_code), None,
                        self._get_persisted_object(arg))
    ...
    def _get_persisted_object(self, object_):
...
if __name__ == '__main__':
    ...
    if data_port != -1:
        data_sender = FunctionCallDataSender(data_port)
